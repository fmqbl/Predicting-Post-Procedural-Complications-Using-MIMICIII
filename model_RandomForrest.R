...
modelrf1 <- randomForest(Class ~ ., data = train2, importance = TRUE, na.action=na.roughfix)
modelrf1
predTrain <- predict(modelrf1, test2, type = "class")
# Checking classification accuracy
cm <-table(predTrain, test2$Class)  
cm
sum(diag(cm))/sum(cm)
...
classifier = svm(formula = factor(Class) ~ .,  data = train2, scale = TRUE,kernel = 'linear') 
print(classifier)
y_pred = predict(classifier, data = test2[-31]) 
length(y_pred)
cm <-table(y_pred, test2$Class)  
cm
sum(diag(cm))/sum(cm)
